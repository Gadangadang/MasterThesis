...
\begin{pmatrix}
    \bf{e}_{T}^{miss} & m_T(j_1) & m_T(j_2) &  m_T(e_1) &  m_T(e_2)\\
    h_L(j_1) & \bf{e_T}(j_1) & m(j_1, j_2) & m(j_1, e_1) & m(j_1, e_2)\\
    h_L(j_2) & h(j_2, j_1) & \delta \bf{e_T}(j_2)& m(j_2, e_1) & m(j_2, e_2)\\
    h_L(e_1) & h(e_1, j_1) & h(e_1, j_2) & \bf{e_T}(e_1) & m(e_1, e_2)\\
    h_L(e_2) & h(e_2, j_1) & h(e_2, j_2) & h(e_2, e_1) & \delta \bf{e_T}(j_2)\\
\end{pmatrix}
\end{equation}
...
and the lower triangular represents longitudal properties. The diagonal has three different properties, $\bf{e_T^{miss}}$, $\bf{e_T}$ and $\delta\bf{e_T}$. 
$\bf{e_T^{miss}}$ is placed in the $(0,0)$ in the matrix. It accounts for the missing energy for the system, which is of high interest for this analysis due to
the search for heavy neutrinos. $\bf{e_T}$ is the transverse energy defined as 
\begin{equation*}
    \bf{e_T} = \sqrt{m^2 + p_T^2}
\end{equation*}
but for light particles such as electrons, this can be approximated to $\bf{e_T} \approx p_T$. $\delta\bf{e_T}$ is the transverse energy imbalance.
It is defined as 
\begin{equation*}
    \delta\bf{e_T} = \frac{E_T(i_n-1) - E_T(i_n)}{E_T(i_n-1) + E_T(i_n)}, \, n = 2, ..., N
\end{equation*}