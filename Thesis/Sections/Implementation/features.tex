...
\marginpar{Mer info om structure}
...
One example of an analysis that have used some features from the RMM is demontrated in \cite{Santos_2017}. The main reason however 
...
matrix, which is optimal when using neural networks. \par
...
electrons etc.), and $N_i$ is the multiplicity of a given object. In the case where the multiplicity of a particle is equal to the number of particles, we can 
...
computational resources. Each cell in the matrix contains information about either single or two particle properties. An example is 
...
\begin{equation*}
    h_L(i_n) = C(\cosh{(y)} - 1),
\end{equation*}
\marginpar{Lang setning} where C is a constant to ensure that the average $h_L(i_n)$ values do not deviate too much from the ranges of the invariant masses 
...
A concequence of using the RMM structure is that the data and SM MC are sparce. This is due to the fact that the RMM allows for 
...
are reconstructed well by the SM MC training samples we use. As described by R. Stuart Geiger et al. \cite{DBLP:journals/corr/abs-2107-02278}, the concept of "Garbage 
...
garbage out. The SM MC simulations are indeed very good, but they are numerical approximations and can 
sometimes \hl{be off}. Thus, every feature that will be used for training have to be checked before being used. This is done by 
comparing the distributions of the features in the SM MC and ATLAS data. SM MC simulations are based on the actual theory 
itself, and comparisons with data taken from ATLAS and other detectors alike are neccesary to prove that the standard model is a good model.\par
Now, if we compare all SM MC and ATLAS data, we would usually expect there to be a good overlap. To ensure that the standard model 
SM MC actualy represents the physics, we create signal and background regions to optimize for amount of signal and amount of background. If we can 
...
we usually conclude that the SM MC is good enough. \marginpar{Lang setning}Now, for this thesis, simply comparing \hl{all ATLAS data} to all SM MC
...
machine learning is getting more popular. The hope is that the signal, whatever it might be, can be revealed with more clever feature 
...
and $m(ele_0, ele_1)$ in the 3 lepton + $e_T^{miss}$ dataset. The $e_T^{miss}$ and $m(ele_0, ele_1)$ satisfy a good ratio between SM MC and ATLAS data, thus we can safely 
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \hfill        
    \caption[2 lepton + $e_T^{miss}$ MonteCarlo and ATLAS data comparison]{Comparison of the SM MC and data for the 2 lepton + $e_T^{miss}$ final state with the features $e_{T}^{miss}$ and flavor composition.
    }
