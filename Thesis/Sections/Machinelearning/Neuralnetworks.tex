...
\begin{table}[H]
    ...
    network found in previous work\cite{FYSSTK}. $\#$ here refers to number of.  }
    \label{tab:notation}
\end{center}
\end{table}
...
\marginpar{Motiver for bruk av gradient descent} Let us now consider a general n-dimensional problem, with parameters $\boldsymbol{\theta} = \{\theta_1, \theta_2, ..., \theta_n\}$. 
...
\end{equation*}
\marginpar{Note that equation start on last layer} We have to use the chain rule to calculate the derivatives, and using that the last layer is $l=L$, we get the derivative with respect to the weights as 
\begin{equation*}
