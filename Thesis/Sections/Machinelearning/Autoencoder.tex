...
\marginpar{Lang setning} It can be useful to think of $q_{\phi}(z|x)$ as a probabilistic encoder, as it for a given data point x will produce a distribution
...
Using Monte Carlo estimates of expectations of a function $f(z)$ with respect to $q_{\theta}(z|x)$, \marginpar{$f(f())$?} Kingman and Welling \cite{VAE} shows that:
\begin{equation}
...
\marginpar{Lacking parameters ?} the latent space be a multivariate Gaussian, lacking parameters: $p_{\theta}(z) = \mathcal{N}(z;0,I)$. We also have that $p_{\theta}(x|z)$ is a multivariate Gaussian,
