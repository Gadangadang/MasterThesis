\section{Autoencoders}\label{sec:autoencder_theory}
...
Mathematically this is represented as follows\cite{weng2018VAE}. Using the annotations of each component in figure \ref{fig:ae_denoise} the decoded information is defined as follows 
\begin{equation*}
    \bf{z} = g_{\phi}(\bf{x}),
\end{equation*}    
...
%  $q_{\phi}(z|x)$ is the probabilistic encoder, which is used to approximate the posterior $p_{\theta}(z|x)$. We now let the prior over 
...
% is a Gaussian with approximately diagonal covariance, we can let the variational approximate posterior be a multivariate Gaussian:
% \begin{equation}
...
% It can be useful to think of $q_{\phi}(z|x)$ as a probabilistic encoder. For a given data point x it will produce a distribution
...
% $p_{\theta}(z|x)$ as a probabilistic decoder, as for a given z it will produce a distribution over possible values for x.

% \subsubsection*{The variational bound}
% Kingman and Welling\cite{VAE} shows that the marginal likelihood can be written as follows:
% \begin{equation}
...
% \footnote{The method constructs an estimator of the marginal likelihood lower bound on the full dataset based on minibatches\cite{VAE}.} 
...
% \begin{equation}
%     \mathbb{E}_{q_{\theta}(z|x^{(i)})}[f(z)] = \mathbb{E}_{p(\epsilon)}[f(g_{\phi}(\epsilon, x^{(i)}))] \approx \frac{1}{L}\sum_{l=1}^{L}f(g_{\phi}(\epsilon^{(l)}, x^{(i)})),
% \end{equation}
...
% An example is the univariate Gaussian distribution. First, let $z \thicksim p(z|x) = \mathcal{N}(\mu, \sigma^2)$. 
