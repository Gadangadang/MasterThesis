...
There are also so-called charged currents, where the sum of charge is not 0. Note that we only require charge conservation,
...
diagrams of possible interactions in proton-proton collisions. Figure \ref{fig:qqbar_ll} displays a lepton pair production
via the Z boson or photon from quark-antiquark annihilation, and figure \ref{fig:ttbar_feynman_example} displays the $t\bar{t}$ production
...
Another note to make is that it has been experimentally verified through observation of neutrino oscillations that the neutrinos are massive\cite{Katrin_neutrinos},
