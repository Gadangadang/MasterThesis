...
class ScaleAndPrep:
    ...
    def mergeDfs(self, exlude=["data18"]) -> None:
        ...
        for file in files:
            ...
            df.drop(['ele3_pt', 'ele3_eta', 'ele3_phi', 'ele3_m', 'muo3_pt', 'muo3_eta', 'muo3_phi', "muo3_m"], axis=1, inplace=True)
...
class RunAE:
    ...
    def getModel(self):
        ...
        x = tf.keras.layers.Dense(units=70,activation="tanh",kernel_regularizer=tf.keras.regularizers.L1(0.05),
            activity_regularizer=tf.keras.regularizers.L2(0.5))(inputs)
        x_ = tf.keras.layers.Dense(units=45,activation="linear")(inputs)
        x1 = tf.keras.layers.Dense(units=20,activation="linear",kernel_regularizer=tf.keras.regularizers.L1(0.05),
        ...
        x = tf.keras.layers.Dense(units=22,activation="relu",kernel_regularizer=tf.keras.regularizers.L1(0.05),
            activity_regularizer=tf.keras.regularizers.L2(0.5))(latent_input)
        x_ = tf.keras.layers.Dense(units=50,activation=tf.keras.layers.LeakyReLU(alpha=1))(x)
        x1 = tf.keras.layers.Dense(units=73,activation="tanh",kernel_regularizer=tf.keras.regularizers.L1(0.05),
            activity_regularizer=tf.keras.regularizers.L2(0.5))(x_)
    ...
    def trainModel(self):
        ...
        with tf.device("/GPU:0"):
            ...
            self.AE_model.fit(self.X_train, self.X_train, epochs=10, batch_size=8192, validation_split=0.2, sample_weight=self.data_structure.weights_train)
    ...
    def checkReconError(self):
        ...
        Zjets2 = self.recon_err_back[np.where(self.data_structure.train_categories == 'Zjets2')]
        diboson2L = self.recon_err_back[np.where(self.data_structure.train_categories == 'diboson2L')]
        diboson3L = self.recon_err_back[np.where(self.data_structure.train_categories == 'diboson3L')]
        diboson4L = self.recon_err_back[np.where(self.data_structure.train_categories == 'diboson4L')]
        triboson = self.recon_err_back[np.where(self.data_structure.train_categories == 'triboson')]
        higgs = self.recon_err_back[np.where(self.data_structure.train_categories == 'higgs')]
        singletop = self.recon_err_back[np.where(self.data_structure.train_categories == 'singleTop')]
        topOther = self.recon_err_back[np.where(self.data_structure.train_categories == 'topOther')]
        Wjets = self.recon_err_back[np.where(self.data_structure.train_categories == 'Wjets')]
        ttbar = self.recon_err_back[np.where(self.data_structure.train_categories == 'ttbar')]


        Zjets2_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'Zjets2')]
        diboson2L_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'diboson2L')]
        diboson3L_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'diboson3L')]
        diboson4L_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'diboson4L')]
        triboson_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'triboson')]
        higgs_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'higgs')]
        singletop_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'singleTop')]
        topOther_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'topOther')]
        Wjets_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'Wjets')]
        ttbar_w = self.data_structure.weights_train[np.where(self.data_structure.train_categories == 'ttbar')]

        

        histo_atlas = [Zjets2 , diboson2L,diboson3L, diboson4L,higgs, singletop, topOther , Wjets, ttbar, triboson ]
        weight_atlas_data = [Zjets2_w , diboson2L_w , diboson3L_w , diboson4L_w ,higgs_w , singletop_w , topOther_w  , Wjets_w , ttbar_w , triboson_w  ]
        ...
        N, bins = np.histogram(self.recon_data, bins = 25, weights=self.data_structure.data_weights)
        ...
        n_bins= bins
        colors = ["green", "magenta", "blue","red", "orange", "brown", "cyan", "mediumorchid", "chartreuse", "gold"]
        labels= ["Zjets2" , "diboson2L", "diboson3L", "diboson4L", "higgs", "singletop", "topOther" , "Wjets", "ttbar", "triboson" ]

        print(np.asarray(labels, dtype=object)[sort_w])

        sns.set_style("darkgrid")
        ax.hist(np.asarray(histo_atlas, dtype=object)[sort_w], 
                n_bins, 
                density=False, 
                stacked=True, 
                alpha=0.5, 
